...
class ServiceManager(
  ...
  def delete(typeSeg: String, idSeg: String)(implicit
    ...
    result.error match {
      ...
      case None    =>
        FormatManager.fromResultsDelete(
          Seq(result),
          FormatManager.resourcesAsNDJSON,
          None,
          sqlAlias
  ...
  def insert(payload: String)(implicit url: Uri): Future[FormattedOutput] =
    Future {
      try {
        ...
        result.error match {
          ...
          case None    =>
            FormattedOutput(
              Seq(result),
              s"$resourceType created successfully",
              akka.http.scaladsl.model.StatusCodes.Created
            )
        }
      } catch {
        case e: Exception =>
          val errorResult = QueryOutput(
            Nil,
            0,
            org.joda.time.DateTime.now(),
            Some(s"Invalid JSON or missing resourceType: ${e.getMessage}"),
            "",
            url.toString()
          )
  ...
  def getEverything(
      patientId: String
  )(implicit url: Uri): Future[FormattedOutput] = {
    // 1. Run the metadata query to find relevant tables/columns
    val metadataQuery = qi.tablesWithColumns(
      Seq("subject", "patient", "beneficiary", "individual")
    )
    ...
    Future {
      ...
      metadataResult.error match {
        case Some(_) =>
          Future.successful(FormatManager.ErrorDefault(Seq(metadataResult)))
        case None =>
          // 2. Generate list of data queries
          val info = metadataResult.queryResults.map(row =>
            (
              row.result.get("table_name").get,
              row.result.get("column_name").get
            )
          )
          ...
          futurePass1Results
            .flatMap { pass1Results =>
              ...
              if (practitionerQueries.nonEmpty) {
                ...
                val futurePass2Results = Future.traverse(practitionerQueries) {
                  query =>
                    Future {
                      qr.runQuery(QueryInput(query, url, url.toString()))
                    }
  ...
  def tablesWithColumns(columns: Seq[String]): Seq[(String, String)] = {
    qr.runQuery(QueryInput(qi.tablesWithColumns(columns), Uri("")))
      ...
      .map(row =>
        (
          row.result.getOrElse("table_name", ""),
          row.result.getOrElse("column_name", "")
        )
      )
      .filter({ case (t, _) => t != "" })
  }
...
object ServiceManager {
  ...
  /** Generates a new UUID string for resource IDs. Uses Java's
    * UUID.randomUUID() which generates a version 4 (random) UUID.
    *
    * @return
    *   A string representation of a UUID (e.g.,
  ...
  /** Generates metadata for a FHIR resource. This will include versionId,
    * lastUpdated, etc.
    *
    * @param uuid
    *   The version ID (typically a UUID)
    * @return
    *   the json object of the Meta tag with versionId and lastUpdated timestamp
    */
  def generateMeta(uuid: String): Obj = {
    Obj(
      "versionId" -> uuid,
      "lastUpdated" -> java.time.ZonedDateTime
        .now()
        .format(
