...
// Main FHIR REST API project (without Spark dependencies)
lazy val root = (project in file("."))
  ...
  .settings(
    ...
    // Exclude sparkbridge directory from main compilation
    Compile / excludeFilter := {
      val default = (Compile / excludeFilter).value
      default || new SimpleFileFilter(f => f.getAbsolutePath.contains("/sparkbridge/"))
    },
    ...
    reStart / javaOptions ++= Seq("--add-opens=java.base/java.nio=ALL-UNNAMED"),
    Test / fork := true,
...
// SparkBridge subproject - separate module with Spark dependencies only
lazy val sparkbridge = (project in file("sparkbridge"))
  .dependsOn(root)
  .settings(
    name := "databricks-fhir-service-sparkbridge",
    conflictWarning := ConflictWarning.disable,
    
    // Point to the sparkbridge source directory in main project
    Compile / unmanagedSourceDirectories := Seq(
      baseDirectory.value / ".." / "src" / "main" / "scala" / "com" / "databricks" / "industry" / "solutions" / "sparkbridge"
    ),
    
    // Only Spark dependencies (provided scope - available at runtime in Databricks)
    libraryDependencies ++= Seq(
      ("org.apache.spark" %% "spark-sql" % sparkVersion % "provided").cross(CrossVersion.for3Use2_13),
      ("org.apache.spark" %% "spark-catalyst" % sparkVersion % "provided").cross(CrossVersion.for3Use2_13)
    )
  )

addCommandAlias("testDocker", ";assembly; testOnly *DockerIntegrationTest")
